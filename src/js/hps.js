...
function HPS(wrp, options) {
  ...
  var setupStyles = function() {
    ...
    applyStyle(self.wrapper, {
      width: (self.options.sectionWidth || deviceWidth) * self.sections.length
    });
...
HPS.prototype.addItems = function addItems({items, itemIterator, onFinish}) {
  const self = this;
  items.map((item, i) => {
